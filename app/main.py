...
def coinManagement():
    if request.method == "PUT":
        ...
        coin = db.get_or_404(Cash, "quarter_usd")
        resp = Response(
            status="204", headers={"X-Coins:": coin.number}, mimetype="application/json"
        )
        coin.number = 0
        db.session.commit()
        return resp
...
def inventoryManagement(idx=-1):
    if request.method == "GET":
        if idx >= 0:
            ...
            resp = Response(
                response=json.dumps(item.inventory),
                status="200",
                mimetype="application/json",
            )
            return resp
            ...
            resp = Response(
                response=json.dumps([x.inventory for x in items]),
                status="200",
                mimetype="application/json",
            )
            return resp
        ...
        if item.inventory > 0:
            if coin.number >= 2:
                ...
                resp = Response(
                    ...
                    headers={
                        "X-Coins:": coin.number,
                        "X-Inventory-Remaining:": item.inventory,
                    },
                    mimetype="application/json",
                )
                coin.number = 0
