...
def generateConnection(autocommit: bool = False) -> psycopg.Connection:
    keys = [
        x.lower() + "=" + getenv(x)
        for x in ["HOST", "USER", "PASSWORD", "DBNAME"]
        if getenv(x)
    ]
...
def createDB(name: str, dropIfExists: bool = False) -> None:
    ...
    if dataBaseExists and dropIfExists:
        cursor.execute(
            psycopg.sql.SQL("DROP DATABASE {};").format(psycopg.sql.SQL(name))
        )
        cursor.execute(
            psycopg.sql.SQL("CREATE DATABASE {};").format(psycopg.sql.SQL(name))
        )
    else:
        cursor.execute(
            psycopg.sql.SQL("CREATE DATABASE {};").format(psycopg.sql.SQL(name))
...
def dropDB(name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT datname FROM pg_catalog.pg_database WHERE datname=%s;", [name]
    )
...
def setupDB() -> None:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "select exists(SELECT 1 FROM pg_tables WHERE tablename='inventory');"
    )
...
def initInventoryTable() -> None:
    ...
    inventory = [
        (i, x, 0, 0.50, 5)
        for i, x in enumerate(["Milk Tea 500mL", "Green Tea 500mL", "Boba Tea 500mL"])
    ]

    with conn.transaction():
        with cursor.copy(
            "COPY inventory (id, name, sold, cost, stock) FROM STDIN;"
        ) as copy:
...
def attempToInputCoin(inputJSON: any, denomination: str = "quarter_usd") -> bool:
    ...
    if "coin" in inputJSON.keys():
        if inputJSON["coin"] >= 1:
            ...
            coins += 1
            cursor.execute(
                "UPDATE cash set available=%s WHERE id=%s;", (coins, denomination)
            )
            conn.commit()
            conn.close()
            out = (True, coins)
